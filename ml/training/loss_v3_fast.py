"""
Loss V3 Fast: Optimized derivative-aware loss function.
...
class CircuitLossV3Fast(nn.Module):
    """
    Loss V3 Fast: Vectorized version with NO Python loops.
    """
    ...
    def compute_validity_penalties_fast(
        ...
        """
        VECTORIZED validity penalties - NO Python loops.
        """
        batch_size, seq_len, num_nodes = node_a_logits.shape
        device = node_a_logits.device
        
        # Soft predictions
        node_a_probs = F.softmax(node_a_logits, dim=-1)  # (B, S, N)
        node_b_probs = F.softmax(node_b_logits, dim=-1)
        type_probs = F.softmax(type_logits, dim=-1)
        
        # Component mask (R, L, C only)
        comp_mask = type_probs[:, :, COMP_R] + type_probs[:, :, COMP_L] + type_probs[:, :, COMP_C]  # (B, S)
        
        # ===== Self-loop penalty (vectorized) =====
        selfloop_prob = (node_a_probs * node_b_probs).sum(dim=-1)  # (B, S)
        selfloop_penalty = (selfloop_prob * comp_mask).mean()
        
        # ===== Duplicate penalty (FULLY VECTORIZED) =====
        # edge_probs[b,s,i,j] = P(edge connects nodes i and j at position s)
        edge_probs = torch.einsum('bsi,bsj->bsij', node_a_probs, node_b_probs)  # (B, S, N, N)
        edge_probs = edge_probs + edge_probs.transpose(-1, -2)  # Symmetric
        
        # Overlap between all pairs of positions
        # overlap[b,s1,s2] = sum over (i,j) of edge_probs[b,s1,i,j] * edge_probs[b,s2,i,j]
        # This is a batched inner product over the (N,N) matrices
        edge_flat = edge_probs.reshape(batch_size, seq_len, -1)  # (B, S, N*N)
        overlap = torch.einsum('bsi,bti->bst', edge_flat, edge_flat)  # (B, S, S)
        
        # Weight by component masks
        weight = comp_mask.unsqueeze(-1) * comp_mask.unsqueeze(-2)  # (B, S, S)
        weighted_overlap = overlap * weight  # (B, S, S)
        
        # Only count upper triangle (s1 < s2)
        triu_mask = torch.triu(torch.ones(seq_len, seq_len, device=device), diagonal=1)  # (S, S)

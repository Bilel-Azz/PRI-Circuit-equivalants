...
class CircuitLossV2(nn.Module):
    ...
    def forward(self, output: dict, batch: dict) -> tuple:
        ...
        # Only compute loss for actual components (R, L, C)
    ...
    def _compute_selfloop_penalty(self, node_a_logits, node_b_logits, mask):
        """P(node_a == node_b) = sum_i P(a=i) * P(b=i)"""
        p_a = F.softmax(node_a_logits, dim=-1)
        p_b = F.softmax(node_b_logits, dim=-1)

        selfloop_prob = (p_a * p_b).sum(dim=-1)

        penalty = (selfloop_prob * mask).sum() / (mask.sum() + 1e-8)
        return penalty

    def _compute_duplicate_penalty(self, node_a_logits, node_b_logits, mask):
        """Penalize duplicate edges using soft edge representation."""
        p_a = F.softmax(node_a_logits, dim=-1)
        p_b = F.softmax(node_b_logits, dim=-1)

        batch_size, seq_len, num_nodes = p_a.shape

        # P(edge = (i, j)) = P(a=i)*P(b=j) + P(a=j)*P(b=i)
        p_edge = torch.einsum('bti,btj->btij', p_a, p_b)
        p_edge_sym = p_edge + p_edge.transpose(-1, -2)

        mask_expanded = mask.unsqueeze(-1).unsqueeze(-1)
        edge_counts = (p_edge_sym * mask_expanded).sum(dim=1)

        # Only penalize edges appearing more than once
        excess = F.relu(edge_counts - 1.0)
    ...
    def _compute_gnd_in_penalty(self, node_a_logits, node_b_logits, type_logits):
        ...
        p_comp = p_type[:, :, COMP_R] + p_type[:, :, COMP_L] + p_type[:, :, COMP_C]

        # Soft approximation of P(node appears in circuit)
        p_node_a_0 = (p_a[:, :, 0] * p_comp).sum(dim=1)
        p_node_b_0 = (p_b[:, :, 0] * p_comp).sum(dim=1)
        p_gnd = 1 - torch.exp(-p_node_a_0 - p_node_b_0)

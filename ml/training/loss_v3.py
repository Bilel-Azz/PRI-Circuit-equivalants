...
class CircuitLossV3(nn.Module):
    """
    Loss V3: Standard loss + derivative matching.
    
    Components:
    - Type classification (CrossEntropy)
    - Node classification (CrossEntropy) 
    - Value regression (MSE)
    - Impedance reconstruction with derivatives (MSE on mag, d1, d2)
    
    Validity penalties (from V2):
    - Self-loop penalty
    - Duplicate edge penalty
    """
    ...
    def __init__(
        ...
        d1_weight: float = 0.5,      # 1st derivative weight
        d2_weight: float = 0.3,      # 2nd derivative weight
    ...
    def compute_derivative_loss(
        ...
        """
        Compute loss on magnitude, 1st and 2nd derivatives.
        
        Args:
            pred_impedance: (batch, 2, num_freq) predicted [log_mag, phase]
            target_impedance: (batch, 2, num_freq) target [log_mag, phase]
        
        Returns:
            Dict with mag_loss, d1_loss, d2_loss, phase_loss
        """
        pred_mag = pred_impedance[:, 0, :]    # (batch, num_freq)
        ...
        # Phase loss (simple MSE, derivatives less important for phase)
    ...
    def compute_validity_penalties(
        ...
        """Compute penalties for invalid circuits (self-loops, duplicates)."""
    ...
    def forward(
        ...
        Args:
            output: Model output dict with logits
            target_seq: (batch, seq_len, 4) target sequence
            target_impedance: (batch, 2, num_freq) target impedance
            pred_impedance: (batch, 2, num_freq) predicted impedance (optional)
        """
        ...
        # Impedance + derivative loss (if provided)
...
if __name__ == "__main__":
    print("=== Testing Loss V3 (Derivative-aware) ===")
    
    # Simulate predictions
    batch_size, seq_len, num_freq = 4, 12, 100
    num_types, num_nodes = 7, 8
    ...
    target_seq = torch.zeros(batch_size, seq_len, 4)
    target_seq[:, :, 0] = torch.randint(0, num_types, (batch_size, seq_len))
    target_seq[:, :, 1] = torch.randint(0, num_nodes, (batch_size, seq_len))
    target_seq[:, :, 2] = torch.randint(0, num_nodes, (batch_size, seq_len))
    target_seq[:, :, 3] = torch.randn(batch_size, seq_len)
    
    # Simulate impedance with resonance
    f = torch.linspace(0, 1, num_freq)
    target_imp = torch.zeros(batch_size, 2, num_freq)
    target_imp[:, 0, :] = 2 - torch.exp(-((f - 0.3) ** 2) / 0.01)  # Resonance dip
    target_imp[:, 1, :] = torch.sin(f * 3.14)
    
    pred_imp = target_imp + 0.1 * torch.randn_like(target_imp)
    
    # Test loss
    loss_fn = CircuitLossV3()
    losses = loss_fn(output, target_seq, target_imp, pred_imp)
    
    print(f"Total loss: {losses['total']:.4f}")
    print(f"Type loss: {losses['type']:.4f}")
    print(f"Magnitude loss: {losses['mag']:.4f}")
    print(f"D1 loss: {losses['d1']:.4f}")
    print(f"D2 loss: {losses['d2']:.4f}")
    print(f"Self-loop penalty: {losses['selfloop']:.4f}")
    print("\nLoss V3 ready!")

...
def is_valid_circuit(components: List[Component]) -> bool:
    ...
    # Build adjacency and check duplicates (same type on same edge)
    type_edges = set()  # (type, node_a, node_b)
    adj = {}
    nodes = set()
    ...
    for c in components:
        ...
        if na == nb:
            return False
        type_edge = (c.comp_type, min(na, nb), max(na, nb))  # Include type
...
def _try_generate_circuit(
    ...
    existing_type_edges = set()  # (type, node_a, node_b)
    components = []
    comp_idx = 0
